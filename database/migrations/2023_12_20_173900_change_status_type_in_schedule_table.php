...
class ChangeStatusTypeInScheduleTable extends Migration
{
    ...
    {
        Schema::table(
            Config::get('filament-database-schedule.table.schedules', 'schedules'),
            function (Blueprint $table) {
                $table->enum('new_status', ['active', 'inactive', 'trashed'])->default('active')->after('status');
            },
        );

        DB::table(Config::get('filament-database-schedule.table.schedules', 'schedules'))->where('status', 0)->update(
            ['new_status' => 'inactive'],
        );
        DB::table(Config::get('filament-database-schedule.table.schedules', 'schedules'))->where('status', 1)->update(
            ['new_status' => 'active'],
        );
    ...
    public function down()
    {
        ...
        DB::table(Config::get('filament-database-schedule.table.schedules', 'schedules'))->where(
            'status',
            'inactive',
        )->update(['old_status' => 0]);
        DB::table(Config::get('filament-database-schedule.table.schedules', 'schedules'))->where(
